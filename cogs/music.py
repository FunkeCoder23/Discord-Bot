...
class YTDLError(Exception):
    pass
# yt-dlp -f 'bv*[height=1080]+ba' --download-archive videos.txt 
# https://www.youtube.com/playlist?list=PLlVlyGVtvuVnUjA4d6gHKCSrLAAm2n1e6 -o 
# '%(channel_id)s/%(playlist_id)s/%(id)s.%(ext)s'
...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    def __init__(self,
        ...
        if not duration:
            return
        self.duration = self.parse_duration(int(duration))
        ...
        while True:
            ...
            try:
                ...
                await self.bot.change_presence(activity=discord.Activity(
                    type=discord.ActivityType.custom, name="Botting"))
                return
        ...
        if voter == ctx.voice_state.current.requester:
            ...
            if total_votes >= votes_needed:
                ...
                return await ctx.send(
                    f'Skip vote added, currently at **{total_votes}/{votes_needed}**'
                )
